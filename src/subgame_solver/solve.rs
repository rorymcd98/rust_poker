...
pub fn solve_cbr_utilties2() {
    ...
    let action_history = &vec![
        Action::Bet,  Action::Bet, Action::Bet,Action::Bet, Action::Call,
        Action::Deal(Card::new(Suit::Clubs, Rank::Ace)), Action::Deal(Card::new(Suit::Spades, Rank::Ace)), Action::Deal(Card::new(Suit::Hearts, Rank::Ace)),
        Action::CheckFold, Action::Bet, Action::Call, 
...
impl HoleCardReaches {
    ...
    pub fn clone_without_card(&self, card: &Card, bulk_multiplier: f64) -> Self {
        let traverser_reaches = self.traverser_reaches.iter().filter(|(hole_cards, _)| hole_cards.0 != *card && hole_cards.1 != *card).map(|(card, reach)| (*card, *reach * bulk_multiplier)).collect();
        let opponent_reaches = self.opponent_reaches.iter().filter(|(hole_cards, _)| hole_cards.0 != *card && hole_cards.1 != *card).map(|(card, reach)| (*card, *reach * bulk_multiplier)).collect();
        Self {
            traverser_reaches,
            opponent_reaches,
        }
...
impl HoleCardPayoffs {
    ...
    pub fn max_subpayoff(&mut self, hole_card_subpayoffs: &HoleCardPayoffs) {
        ...
        for (hole_card, payoff) in hole_card_subpayoffs.traverser_payoffs.iter() {
            self.traverser_payoffs.entry(*hole_card)
    ...
    pub fn showdown_payoffs(player_pot: u8, traverser_hand_rankings: HashMap<(Card, Card), u16>, opponent_hand_rankings: HashMap<(Card, Card), u16>, hole_card_reaches: &HoleCardReaches) -> HoleCardPayoffs {        
        // Calculate total opponent weight for normalization
        let mut payoffs = HoleCardPayoffs::default();
        ...
        for (traverser_hole_cards, traverser_reach) in hole_card_reaches.traverser_reaches.iter() {
            ...
            payoffs.traverser_payoffs.insert(*traverser_hole_cards, ev * traverser_reach);
        }
        payoffs
    }
}
...
impl<'a> CbvSubTree<'a> {
    pub fn explore_root(&mut self, ) -> HashMap<(Card, Card), f64> { // TODO - move Vec<Action> to the struct
        ...
        debug_assert!(self.action_history == initial_action_history, "Action history should not have changed");
        ...
        let mut sub_trees: Vec<&mut CbvSubTree<'a>> = self.subtrees.iter_mut().map(|(_, tree)| tree).collect::<Vec<_>>();

        sub_trees.par_iter_mut().progress_with(pb).for_each(|subtree| {
    ...
    fn get_computed_payoffs(&mut self) -> HoleCardPayoffs {
        let mut subtree = self.subtrees.remove(&self.action_history.clone()).expect("Should have found a matching sub-tree");
        subtree.hole_card_payoffs.take().expect("Should not be getting payoffs for a node that has not been computed")
    }
    ...
    pub fn evaluate_fold(&self, reaches: &HoleCardReaches, current_player: &Player, traverser_pot: u8, opponent_pot: u8) -> HoleCardPayoffs {
    ...
    pub fn evaluate_showdown(&self, reaches: &HoleCardReaches, current_player_pot: u8) -> HoleCardPayoffs {
        ...
        HoleCardPayoffs::showdown_payoffs(current_player_pot, traverser_hand_rankings, opponent_hand_rankings, reaches)
    }

    fn hole_card_rankings_player(&self, reaches: &HoleCardReaches, player: Player, board_cards: &[Card; 5]) -> HashMap<(Card, Card), u16> {
        let player_hole_cards = reaches.get_hole_cards(player);
        self.hole_card_rankings(&player_hole_cards, board_cards)
    }

    fn hole_card_rankings(&self, hole_cards: &Vec<&(Card, Card)>, board_cards: &[Card; 5]) -> HashMap<(Card, Card), u16>  {
    ...
    fn traverse_deal(&mut self, reaches: &HoleCardReaches, depth: usize) -> HoleCardPayoffs {
        ...
        for card in potential_next_cards {
            ...
            let subpayoffs = self.explore_sub_trees(&reaches.clone_without_card(&card, reach_multiplier), depth);
