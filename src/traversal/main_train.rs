...
use super::strategy::strategy_hub::{
    deserialise_strategy_hub, serialise_strategy_hub, StrategyHub, StrategyPair,
};
...
pub fn begin_tree_train_traversal() {
    ...
    let strategy_hub: StrategyHub<TrainingStrategy> =
        Arc::try_unwrap(strategy_hub).expect("Arc has more than one strong reference");
    // validate_strategy_map::<TrainingStrategy>(&strategy_hub.into_map());
    serialise_strategy_hub(BLUEPRINT_FOLDER, strategy_hub)
        .expect("Failed to serialise strategy hub");
}
...
pub fn get_unique_cards(sb_key: &StrategyHubKey, bb_key: &StrategyHubKey) -> [Card; 4] {
    ...
    with_rng(|rng| {
        let bb_cards = valid_combinations
            .choose(rng)
        ...
        for i in 1..TRAIN_ITERATIONS {
            ...
            for player in players {
                let deal = match player {
                    Player::Traverser => {
                        new_random_nine_card_game_with(cards[0], cards[1], cards[2], cards[3])
                    }
                    Player::Opponent => {
                        new_random_nine_card_game_with(cards[2], cards[3], cards[0], cards[1])
                    }
                };
                let mut branch_traverser = TrainingBranchTraverser::new(
                    &mut sb_branch,
                    &mut bb_branch,
...
fn load_or_create_strategy_hub() -> StrategyHub<TrainingStrategy> {
    match deserialise_strategy_hub(BLUEPRINT_FOLDER) {
        ...
        Err(err) => {
            println!(
                "Could not deserialise an existing strategy-hub, creating new strategy hub: {}",
                err
            );
...
pub fn get_all_combos_by_blind(is_smallblind: bool) -> Vec<StrategyHubKey> {
    ...
    sb_elements.extend(all_pocket_pairs().iter().map(|rank| StrategyHubKey {
        low_rank: rank.0,
        high_rank: rank.1,
        is_sb: is_smallblind,
        is_suited: false,
...
fn create_new_all_cards_strategy_hub() -> StrategyHub<TrainingStrategy> {
    ...
    let bb_elements = sb_elements
        .clone()
        .into_iter()
        .map(|element| StrategyHubKey {
            is_sb: false,
            ..element
        })
        .collect::<Vec<StrategyHubKey>>();
...
impl<'a> TrainingBranchTraverser<'a> {
        ...
        match self.game_state.current_player {
            ...
            Player::Traverser => {
                ...
                for action in 0..num_available_actions {
                    utilities[action] = self.traverse_chosen_action(
                        action,
                        previous_player,
                        pot_before_action,
                        bets_before_action,
    ...
    fn get_strategy(&mut self) -> &mut TrainingStrategy {
        let strategy_branch =
            if self.game_state.current_player == self.game_state.small_blind_player {
                &mut self.sb_strategy_branch
            } else {
                &mut self.bb_strategy_branch
            };
        strategy_branch.get_or_create_strategy(
            self.game_state.serialise_history_with_current_player(),
            self.game_state.get_num_available_actions(),
        )
    }
}
