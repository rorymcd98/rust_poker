...
pub fn get_straight_abstraction(
    ...
    if highest_straight_with_gutshot > highest_straight_without_gutshot {
        Some(StraightAbstraction {
            bucketed_high_card: round_rank(gutshot_high_card),
            cards_in_straight: highest_straight_with_gutshot,
            requires_gutshot: true,
        })
    } else if highest_straight_without_gutshot > 0 {
        return Some(StraightAbstraction {
            bucketed_high_card: round_rank(connected_high_card),
            cards_in_straight: highest_straight_without_gutshot,
    ...
    if counts[2] > 0 {
        ...
        return Some(ConnectedCardsAbstraction::ThreeOfAKind(
            ThreeOfAKindAbstraction {
                toak_order_score: seen_so_far_toak.max(2),
            },
        ...
        return Some(ConnectedCardsAbstraction::TwoPair(TwoPairAbstraction {
            two_pair_order_score: seen_so_far_pair.max(2),
        }));
    } else if counts[0] > 0 {
        return Some(ConnectedCardsAbstraction::Pair(PairAbstraction {
            pair_order_score: seen_so_far_pair.max(2),
        }));
    } else {
        return None;
    }
